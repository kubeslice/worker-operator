...
var _ = FDescribe("Gateway redundancy test scenario", func() {
	...
	var slice, createdSlice *kubeslicev1beta1.Slice
...
- 10.96.0.0/12`)
		...
		It("Should update cluster CR with List of Node IPs", func() {
			...
			err = hub.PostClusterInfoToHub(ctx, k8sClient, k8sClient, "cluster-gw", "kubeslice-cisco", nodeIP)
			Expect(err).To(BeNil())
			...
			Expect(err).To(BeNil())
			deploykey := types.NamespacedName{Name: "test-slicegw-pod", Namespace: CONTROL_PLANE_NS}
			Eventually(func() bool {
