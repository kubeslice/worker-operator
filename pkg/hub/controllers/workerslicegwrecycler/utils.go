...
func (r *Reconciler) verify_new_deployment_created(e *fsm.Event) error {
	// we need to verify the number of deployments, shouold be 2 on both client and server and wait till the new pod is up and running and update this new pods name in workerslicegwrecycler object and move the fsm to new state = new_deployment_created
...
func (r *Reconciler) update_routing_table(e *fsm.Event) error {
	ctx := context.Background()
	log := logger.FromContext(ctx).WithName("workerslicegwrecycler").WithName("update_routing_table logger")
...
func (r *Reconciler) delete_old_gw_pods(e *fsm.Event) error {
	ctx := context.Background()
	log := logger.FromContext(ctx).WithName("workerslicegwrecycler").WithName("delete_old_gw_pods logger")
