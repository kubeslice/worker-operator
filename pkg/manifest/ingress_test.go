...
var _ = Describe("Ingress Manifest", func() {
	var (
		ctx        context.Context
	...
	Context("With InstallIngress function", func() {
		It("Should install all ingress resources successfully in the slice", func() {
			err := manifest.InstallIngress(ctx, fakeClient, slice)
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(deploy.ObjectMeta.Name).Should(Equal("green-istio-ingressgateway"))
		})
	...
	Context("With UninstallIngress function", func() {
		BeforeEach(func() {
			err := manifest.InstallIngress(ctx, fakeClient, slice)
			Expect(err).NotTo(HaveOccurred())
		})

		It("Should uninstall all ingress resources successfully from the slice", func() {
			err := manifest.UninstallIngress(ctx, fakeClient, "green")
			Expect(err).NotTo(HaveOccurred())
